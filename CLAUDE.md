...
- `make generate PRESET=tiny|dev|full [SEED=N]` — `hirestream generate backfill --overwrite`: **replaces** generated source data under `data/lake/` (HRIS in `bronze/hris/`, manifest in `_runs/<run_id>/`)
